...
def verify_result(question: str, sql: str, results: list, columns: list) -> dict:
    ...
    Uses fast model. Returns {"is_valid": bool, "issue": str | None}
    """
    from app.core.groq_client import call_llm

    prompt = f"""User asked: "{question}"
SQL executed: {sql[:400]}
Result columns: {columns}
Result preview (first 5 rows): {results[:5]}
Number of rows returned: {len(results)}

Quick check — does this result answer the question?
Common issues:
- Did SQL return the right columns for what was asked?
- If they asked for "top 5", are there 5 rows?
- If they asked about a specific region/period, is it filtered?
- Does magnitude make sense? (revenue shouldn't be negative)

Return JSON: {{"is_valid": true, "issue": null}} or {{"is_valid": false, "issue": "description of what's wrong"}}"""

    try:
        return call_llm("smart_sql", "Verify this SQL result. Return only JSON.", prompt, json_mode=True)
    except Exception:
