...
1. What metric is being asked? (identify the aggregation: SUM, COUNT, AVG, COUNT DISTINCT)
2. What filters are mentioned? (region, time period, status, category — add WHERE for each)
3. What grouping is needed? (GROUP BY what dimension?)
4. What ordering is needed? (TOP N = ORDER BY + LIMIT)
5. What tables need to be joined?

Write your reasoning in the "reasoning" field, then write the SQL.

## EXACT METRIC DEFINITIONS:
- "total revenue" / "sales" → SUM(amount)
- "number of orders" / "how many orders" → COUNT(order_id)
- "average order value" / "AOV" → AVG(amount)
...
11. WARNING: String values are CASE SENSITIVE in SQLite. Use EXACT values as shown in schema sample values.
...
{{
  ...
  "reasoning": "step-by-step explanation of metric, filters, grouping, joins"
}}
"""
